...
class WP_Site_Alias_Query {
	...
	protected function get_alias_ids() {
		...
		if ( ! empty( $alias_id ) ) {
			$this->sql_clauses['where']['ID'] = $wpdb->prepare( 'id = %d', $alias_id );
		}
		...
		$this->sql_clauses['from']    = "FROM {$wpdb->blog_aliases} {$join}";
		...
		$alias_ids = $wpdb->get_col( $this->request );
	...
	private function set_found_site_aliases( $alias_ids = array() ) {
		...
		if ( ! empty( $this->query_vars['number'] ) && ! empty( $this->query_vars['no_found_rows'] ) ) {
			...
			$this->found_site_aliases = (int) $wpdb->get_var( $found_site_aliases_query );
	...
	protected function parse_orderby( $orderby ) {
		...
		switch ( $orderby ) {
			...
			case 'alias__in':
				...
				$parsed = "FIELD( {$wpdb->blog_aliases}.id, $alias__in )";
				break;
			case 'site__in':
				$site__in = implode( ',', array_map( 'absint', $this->query_vars['site__in'] ) );
				$parsed = "FIELD( {$wpdb->blog_aliases}.blog_id, $site__in )";
				break;
			case 'domain':
			case 'created':
			case 'status':
